'use client'

import { useActionState } from "react";
import { createEntry } from "@/actions/journal-actions";
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/textarea";
import { Loader2, Send } from "lucide-react"; // Pastikan install lucide-react jika belum

const initialState = {
  message: "",
  success: false,
};

export function CreateEntryForm() {
  const [state, formAction, isPending] = useActionState(createEntry, initialState);

  return (
    <form action={formAction} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden group focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
      <div className="relative">
        <Textarea 
          name="content" 
          placeholder="Apa yang ada di pikiranmu saat ini?" 
          className="min-h-[100px] border-none shadow-none resize-none p-0 text-base focus-visible:ring-0 placeholder:text-slate-400 bg-transparent"
          required
        />
      </div>
      
      <div className="flex justify-between items-center mt-3 pt-2 border-t border-slate-50">
        <div className="text-xs text-slate-400">
           {state.message && !state.success && (
            <span className="text-red-500">{state.message}</span>
          )}
        </div>
        
        <Button 
          type="submit" 
          disabled={isPending}
          size="sm"
          className="rounded-full bg-indigo-600 hover:bg-indigo-700 text-white px-6 transition-all"
        >
          {isPending ? (
            <Loader2 className="h-4 w-4 animate-spin" />
          ) : (
            <>
              Simpan <Send className="ml-2 h-3 w-3" />
            </>
          )}
        </Button>
      </div>

      {state.success && (
        <div className="absolute inset-0 bg-white/90 flex items-center justify-center backdrop-blur-sm z-10 animate-in fade-in">
          <p className="text-green-600 font-medium flex items-center gap-2">
            ✅ Tersimpan!
          </p>
        </div>
      )}
    </form>
  );
}